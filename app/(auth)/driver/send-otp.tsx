 ...
 * Phone number input with country code selector and OTP sending for drivers
 */
...
import {
  View, ScrollView, KeyboardAvoidingView, Platform, TouchableOpacity, ActivityIndicator } from "react-native";
...
export default function DriverSendOtpScreen() {
  ...
  const handleSendOtp = async () => {
    ...
    setLoading(true);
    try {
      const response = await sendOtp(fullPhone, "driver");

      if (response.success && response.data) {
        // Navigate to verify OTP screen with phone number
        router.push({
          pathname: "/(auth)/driver/verify-otp",
